...
<div class="container-fluid py-4">
    <form action="{{ route('transaction.inbStore') }}" method="POST" id="inbound-form">
        @csrf
                    ...
                    <div class="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center gap-3">
                        <div>
                            ...
                            <p class="text-secondary mb-0">
                                Create inbound header and add item details with a simple workflow.
                            </p>
                        </div>
            ...
            <div class="col-12">
                <div class="card soft-card mb-4 mx-2 mx-md-4">
                    <div class="card-header">
                        ...
                        {{-- <p class="section-subtitle">Fill in the main inbound information before adding detail rows.</p> --}}
                    </div>

                    <div class="card-body pt-4">
                        <div class="row g-4">
                            <div class="col-md-4">
                                <label class="form-label form-label-modern">Inbound Code</label>
                                <input
                                    type="text"
                                    name="inb_code"
                                    class="form-control"
                                    value="{{ old('inb_code') }}"
                                    placeholder="Example: INB-2026-0001"
                                    required
                                >
                            </div>
                            ...
                            <div class="col-md-4">
                                ...
                                <select name="ref_wh_id" class="form-select" required>
                                    <option value="">Choose warehouse</option>
                                    @foreach ($warehouses as $warehouse)
                                        <option value="{{ $warehouse->wh_id }}"
                                            {{ old('ref_wh_id') == $warehouse->wh_id ? 'selected' : '' }}>
                                            {{ $warehouse->wh_name }}
                                            @if(!empty($warehouse->wh_code))
                                                - {{ $warehouse->wh_code }}
                                            @endif
                                        </option>
                                    @endforeach
                            ...
                            <div class="col-md-4">
                                ...
                                <select name="inb_stat" class="form-select" required>
                                    <option value="">Choose status</option>
                                    <option value="DRAFT" {{ old('inb_stat') == 'DRAFT' ? 'selected' : '' }}>DRAFT</option>
                                    <option value="RECEIVED" {{ old('inb_stat') == 'RECEIVED' ? 'selected' : '' }}>RECEIVED</option>
                                    <option value="PARTIAL" {{ old('inb_stat') == 'PARTIAL' ? 'selected' : '' }}>PARTIAL</option>
                                </select>
                            </div>
                    ...
                    <div class="card-header">
                        <div class="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center gap-3">
                            <div>
                                ...
                                {{-- <p class="section-subtitle">Add product rows, choose location, and define quantities.</p> --}}
                            </div>

                            <button type="button" class="btn btn-premium-soft" id="btn-add-detail-row">
...
@include('transaction.inbound.create.ext_inbCreate')
@endsection